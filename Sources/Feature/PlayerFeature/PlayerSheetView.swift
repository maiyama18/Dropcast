...
#Preview {
    let playerState: SoundPlayerState = {
        let playerState = SoundPlayerState(
            context: CloudKitPersistentProvider.preview.viewContext
        )
        playerState.state = .playing(episode: .fixture(context: CloudKitPersistentProvider.preview.viewContext))
        return playerState
    }()
